...
> 寫成「期望值」,測試從此永遠是綠的。
>
> **只有 `cmn-Hant` 需要樣本**,少了會讓測試紅掉 —— 繁中是這個 fork 存在的理由。
> `en` / `ru` / `ko` 由上游維護,替它們建樣本不是這裡的工作(與 `verify-i18n`
> 把 ko / ru 標成「非本專案維護」是同一套分野);真的補了樣本會自動納入回歸網。
>
> ⚠ `renderer/test/fixtures/**` 在 `.gitattributes` 裡設成 `-text`,**不要拿掉**。
> fixture 是逐位元組的剪貼簿內容,而快照存了 `rawText`;讓 autocrlf 改寫行尾的話,
> 同一份程式碼在 commit 前後就會跑出不同結果(實際發生過,三件全部報 rawText 不符)。
>
